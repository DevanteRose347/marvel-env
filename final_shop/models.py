...
class Product(db.Model):
    ...
    comic_book= db.Column(db.String(100), nullable = False)
    ...
    def __init__(self, name, edition, comic_book, movie, price, quantity, image = "", description = ""):
...
class Customer(db.Model):
    ...
    prodord = db.relationship(
        "ProdOrder", backref="customer", lazy=True
    )  # backref is just how are these related, lazy means a Customer can exist without the ProdOrder table
...
products_schema = ProductSchema(
    many=True
)  # this for passing multiple products, list of dictionaries
