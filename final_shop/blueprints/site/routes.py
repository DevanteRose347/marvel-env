...
from final_shop.forms import ProductForm
...
def shop():
    ...
    shop_stats = {
        "products": len(shop),
        "sales": sum(
            order.order_total for order in orders),  # order totals was total cost of that specific order
...
def create():
    ...
    if request.method == "POST" and createform.validate_on_submit():
        ...
        edition = createform.edition.data
        comic_book = createform.comic_book.data
        movie = createform.movie.data
        ...
        shop = Product(
            name, edition, comic_book, movie, price, quantity, image, desc
...
def update(id):
    ...
    if request.method == "POST" and updateform.validate_on_submit():
        try:
            ...
            flash(
                f"You have successfully updated product {product.name}",
                category="success",
            )
            return redirect("/")
